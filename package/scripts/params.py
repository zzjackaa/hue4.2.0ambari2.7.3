...
solr_master_hosts = default("/clusterHostInfo/infra_solr_hosts", [])
solr_master_hosts.sort()
solr_url='http://localhost:8886/solr/'
if len(solr_master_hosts) > 0:
  solr_port = config['configurations']['infra-solr-env']['infra_solr_port']
  #solr_znode = config['configurations']['solr-config']['solr.znode']
  solr_master_host = solr_master_hosts[0]
  solr_url = format('http://' + solr_master_host + ':' + str(solr_port) +  '/solr/')
...
hbase_thrift_hosts = default("/clusterHostInfo/hbase_thrift_hosts", [])
hbase_clusters = []
hbase_cluster = ''
if len(hbase_thrift_hosts) > 0:
  hbase_master_port=default('configurations/hbase-site/hbase.regionserver.thrift.port', 9090)
  for i in range(len(hbase_master_hosts)):
    hbase_clusters.append(format("(Cluster" + str(i+1) + "|" + str(hbase_thrift_hosts[i]) + ":" +str(hbase_master_port) + ")"))
...
if len(spark_thriftserver_hosts) > 0:
  ...
  spark_hiveserver2_thrift_port = str(config['configurations']['spark2-hive-site-override']['hive.server2.thrift.port']).strip()
